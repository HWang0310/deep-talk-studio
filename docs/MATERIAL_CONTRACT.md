# Material Search & Visual Assistance 0.5 契约
...
正式 Artifact 包括身份、revision、时间、模式、状态、Script ID/revision/digest/Review ID、Research ID/revision、Profile 版本、Cue Sheet、Materials、Generated Visuals、Gaps、Research Update、Warnings、Provider provenance、Review linkage 和 Package digest。
...
Rights manifest 必须保存实际打开的权利页面、依据、许可证 URL（明确复用、CC、press asset 必填）、检查时间和工具引用。不能从“官方网站”“新闻媒体”“公开视频”等名称推断许可。

最终资格由程序决定：真实检查且权利属于前四种的静态素材才可 `ready_to_use`；普通新闻或未知权利为 `reference_only`；需许可为 `permission_required`；伪造、错配或应避免素材为 `rejected`。
...
网页/PDF 截图通过本地静态 capture 登记进入包，必须保留原页面、页码、截取区域、上下文、caption、能证明什么和不能证明什么。PDF 优先截相关页，不把整份文件当“已经证明”。
...
- Timeline 日期和 Claim 组合必须存在于批准的 Research timeline。
- 数值图的每个数值必须能在绑定 Claim 文本中找到。
- 所有 Claim/Evidence/Beat/Node 引用必须存在并对应。
...
Issue ID、严重度、阻断数量、item eligibility、package Gate 和最终状态都由代码推导并在读取时重验。危险 item 可以隔离；若有安全替代，包为 `reviewed_with_warnings`，包级伪造或无安全方案则 `blocked`。
