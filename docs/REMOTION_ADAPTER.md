...
- 四类 Motion 直接消费 `scene_payload`：timeline baseline/marker、bar baseline grow/label/value、comparison mechanism cards、diagram node/edge。Comparison label 单次显示、两个 fact block 同卡；Diagram node 使用受限 foreignObject 安全换行，edge label 使用独立背景 plate。它们使用 React/SVG 原子元素，不使用 V0.5 整图 SVG 或 CanvasImage。
