...
def mux_clean_aroll_audio(visual_path,media,output_path):
 ...
 # Copy the original stream with its input timestamps; no trim/reset/shortest/edit filter.
 command=["ffmpeg","-v","error","-nostdin","-copyts","-i",str(visual_path),"-copyts","-i",str(source),"-map","0:v:0","-map","1:a:0","-c:v","copy","-c:a","copy","-avoid_negative_ts","disabled",str(output)]
 _run(command);src=probe_audio_presentation(source);preview=probe_audio_presentation(output);tol=max(src.tolerance_seconds,preview.tolerance_seconds,1/30);validate_preview_audio_presentation(src,preview,tol)
 return AudioMuxResult(output,"ffmpeg presentation-preserving stream copy",preview.audio_stream_count,float(json.loads(_run(["ffprobe","-v","error","-show_entries","format=duration","-of","json",str(output)]).stdout)["format"]["duration"]),preview.audio_start_seconds,preview.audio_end_seconds,preview.internal_gaps,tol,True,hashlib.sha256(output.read_bytes()).hexdigest(),output.stat().st_size)
def build_aligned_preview_manifest(path,bridge,profile,media,used_placement_ids):
 value={"artifact_version":"aligned-preview-manifest/1","bridge_id":bridge["bridge_id"],"bridge_digest":bridge["package_digest"],"profile_digest":profile["profile_digest"],"media_id":media["media_id"],"media_sha256":media["sha256"],"source_presentation_evidence_digest":media["presentation_evidence"]["evidence_digest"],"used_placement_ids":list(used_placement_ids),"output_sha256":hashlib.sha256(Path(path).read_bytes()).hexdigest(),"output_byte_size":Path(path).stat().st_size}
