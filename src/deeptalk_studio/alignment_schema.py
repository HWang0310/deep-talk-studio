...
GAP_SCHEMA = _object(
    {
        "gap_id": _string(),
        "gap_type": _enum([
            "omitted_script_span", "ad_lib_transcript_span", "trailing_ad_lib_transcript_span", "repeated_or_ambiguous_span",
...
GLOBAL_SCRIPT_UNIT_SCHEMA = _object(
    {
        "script_token_index": _integer(), "script_char_start": _integer(), "script_char_end": _integer(1),
        "operation": _enum(["primary_match", "numeric_match", "substitution", "script_deletion"]),
        "transcript_token_index": _integer(-1), "transcript_unit_id": _string(allow_empty=True),
        "actual_start_seconds": _DECIMAL, "actual_end_seconds": _DECIMAL,
    }
)

GLOBAL_INSERTION_SCHEMA = _object(
    {
        "transcript_token_index": _integer(), "transcript_unit_id": _string(),
        "actual_start_seconds": _DECIMAL, "actual_end_seconds": _DECIMAL,
        "left_script_token_index": _integer(-1), "right_script_token_index": _integer(-1),
        "ownership": _enum(["leading", "beat_local", "beat_boundary", "trailing"]),
        "owner_beat_id": _string(allow_empty=True),
    }
)

GLOBAL_MAPPING_SCHEMA = _object(
    {
        "mapping_version": _enum(["global-monotonic-projection/1"]), "trace_digest": _DIGEST,
        "script_token_count": _integer(1), "transcript_token_count": _integer(1),
        "ambiguity_code": _enum(["none", "ambiguous_match"]),
        "script_units": _array(GLOBAL_SCRIPT_UNIT_SCHEMA), "transcript_insertions": _array(GLOBAL_INSERTION_SCHEMA),
    }
)

SCRIPT_ALIGNMENT_SCHEMA = _object(
    {
        "artifact_version": _enum(["script-alignment/1", "script-alignment/2"]),
        ...
        "global_mapping": GLOBAL_MAPPING_SCHEMA,
