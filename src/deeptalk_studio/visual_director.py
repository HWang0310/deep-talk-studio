...
def build_visual_director_plan(roots, proposals, *, plan_id, created_at, revision=1, previous_revision=0):
    ...
    if roots.get("timing_provenance") != "actual_aroll_alignment":
        raise VisualDirectorError("Visual Director 只能使用真实 A-roll Alignment，不得使用估算时间")
    opportunities = []
    seen = set()
    for raw in proposals:
        ...
        item = {"opportunity_id": raw["opportunity_id"], "cue_id": raw["cue_id"], "source_time_range": {"start_seconds": start, "end_seconds": end}, "visual_intent": raw["visual_intent"], "why_visual": raw["why_visual"], "decision": decision, "importance": raw.get("importance", "supporting"), "review_requirement": "advanced_spec_review" if decision == "ADVANCED_MOTION" else "not_needed", "risk_flags": list(raw.get("risk_flags", [])), "alignment_digest": roots["alignment_digest"], "status": "keep" if decision == "KEEP_A_ROLL" else "proposed"}
