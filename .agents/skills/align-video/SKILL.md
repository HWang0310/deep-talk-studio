...
For a real run, call `resolve_real_edit_bridge_session(...)` and then the single `run_real_edit_bridge_session(...)` production entrypoint. V1 automatically prepares and uses the repository-owned local `whisper.cpp` v1.9.2 full multilingual `large-v3` runtime/model with `--dtw large.v3`; the normal user does not install it, choose a provider, set `OPENAI_API_KEY`, or handle a model URL. If first use needs the model, explain only that the local speech model is being prepared and downloaded once. Never silently use medium, turbo, quantized models or cloud transcription. Cloud transcription remains an internal future optional adapter, never the V1 preflight gate. The entrypoint owns immutable import, evidence-preserving transcription audio, Mapping, Chunk Plan, timestamp-capable transcription, Alignment, Basic Subtitle V1 Artifact/SRT, Material projection, Placement timing, Edit Bridge, burned-in subtitled Preview, original Clean A-roll audio mux, Manifest and canonical QA. Never assemble stage lambdas, interpolate segment timestamps, disable the bound subtitles, mix B-roll audio, or modify reviewed history.
