...
- Runtime records: separate inspection and rights manifests. Only actual page opens enter inspection. Safe reuse requires a `rights_evidence_url` actual-open record whose tool reference exactly matches the rights record.
...
- Original visuals: timeline/bar/comparison/diagram, 1920×1080, source-backed IDs, attribution, safe area, duration, animation intent and future render hints. Nested refs are independently grounded; a top-level ref never certifies an ungrounded child.
- Review: inspect the existing package only; do not expand research. Unsafe items can be isolated if safe alternatives remain. r1 input/inspection/rights artifacts plus the Review Artifact must deterministically rebuild any reviewed r2.
