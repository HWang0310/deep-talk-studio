...
def create_integrated_roots(root:Path):
 report,script,_=reviewed_inputs();profile=load_material_profile();asset_root=root/"material-assets"/"MAT-INTEGRATED"
 image=asset_root/"real"/"date.png";image.parent.mkdir(parents=True);subprocess.run(["ffmpeg","-v","error","-y","-f","lavfi","-i","color=c=navy:s=320x180:d=0.1","-frames:v","1",str(image)],check=True)
 ranged=build_media_fixture(asset_root/"real",MediaFixtureSpec(name="ranged",duration="2"));unranged=build_media_fixture(asset_root/"real",MediaFixtureSpec(name="unranged",duration="2"))
 rows=[
 ...
 for item,path in zip(data["materials"],files):item.update(local_path=str(path),byte_size=path.stat().st_size,sha256=hashlib.sha256(path.read_bytes()).hexdigest())
 ...
 data=package.to_dict();data["package_digest"]=material_package_digest(data);package=MaterialPackage(data);packages=root/"material-packages";r1=save_material_package(package,packages)
 ...
 motion_dir=root/"production-assets"/"PROD-INTEGRATED"/"assets";motion_dir.mkdir(parents=True);outputs=[]
 ...
 plan_path=save_production_plan(plan,root/"production-packages");save_production_artifact(manifest_result.manifest,plan_path,"motion-asset-manifest-r0001.json");save_production_artifact(qa,plan_path,"production-qa-r0001.json")
