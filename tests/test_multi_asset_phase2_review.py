...
class Phase2ReviewOrchestrationTests(unittest.TestCase):
    ...
    def test_real_fake_subprocess_failure_isolation_and_canonical_portfolio_shape(self):
        with tempfile.TemporaryDirectory() as root:
            artifact = orchestrate_candidate_portfolio([O], config([("MG","suitable",True),("Illustrated","unavailable",True),("Hand","abstain",True),("Disabled","suitable",False)]), production_profile="RICH", policy=load_candidate_generation_policy(ROOT / "config/candidate-generation-profile.json"), job_root=Path(root))
    ...
    def test_rich_and_standard_no_suitable_run_real_generation_for_every_eligible_fake_plugin(self):
        with tempfile.TemporaryDirectory() as root:
            rich=orchestrate_candidate_portfolio([O],config([("A","suitable",True),("B","suitable",True),("C","borderline",True)]),production_profile="RICH",policy=load_candidate_generation_policy(ROOT / "config/candidate-generation-profile.json"),job_root=Path(root))
            self.assertEqual(len(ready_candidates([rich])),3)
        with tempfile.TemporaryDirectory() as root:
            standard=orchestrate_candidate_portfolio([O],config([("B","borderline",True),("A","borderline",True)]),production_profile="STANDARD",policy=load_candidate_generation_policy(ROOT / "config/candidate-generation-profile.json"),job_root=Path(root))
    ...
    def test_execution_evidence_has_runtime_version_config_digest_and_locators(self):
        with tempfile.TemporaryDirectory() as root:
            artifact=orchestrate_candidate_portfolio([O],config([("A","suitable",True)]),production_profile="LEAN",policy=load_candidate_generation_policy(ROOT / "config/candidate-generation-profile.json"),job_root=Path(root))
