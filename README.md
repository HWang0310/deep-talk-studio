...
The next accepted product direction is **Multi-Asset Studio**: `Semantic Timeline → Visual Opportunity → non-exclusive Candidate Portfolio → Candidate Asset Pack → Multi-option Edit Map → creator selection`. Its ecosystem is multi-repo and plugin-first: families evolve independently behind a stable Core boundary. Visual Asset Plugin Contract V1 is ACCEPTED_UNRELEASED architecture and its Multi-Asset Implementation Plan is accepted; V2 runtime schemas and implementation have **not started**. The next gate is Phase 0 — Contract fixtures + frozen V1 compatibility baseline — in a separate implementation session, branch, and review.
