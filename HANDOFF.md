...
AIGC:
  ...
  Label: '1'
  ProduceID: '44b48fb5-e1d6-4a0b-8d61-1caa32075c1b'
  PropagateID: '44b48fb5-e1d6-4a0b-8d61-1caa32075c1b'
  ReservedCode1: '65947447-b0e9-4822-ace5-aa236e7db5d6'
  ReservedCode2: '65947447-b0e9-4822-ace5-aa236e7db5d6'
---
...
- **Conflict resolution:** `AGENTS.md` was the only content conflict. Resolution preserved main-only canonical roster history and all five current roles — Nexus, Atlas, Forge, Scribe, Orion — alongside the development branch's current end-of-round documentation rules. No blanket ours/theirs, force push, or branch-pointer substitution was used. Two pre-existing whitespace defects in merged documentation/evaluation text were removed so the RC can satisfy `git diff --check`.
- **Product definition:** v1.0 is one main + four auxiliaries: writing; source-backed insert materials; explicit MG; explicit Illustrated Metaphor / 小黑漫画; explicit Hand-drawn Animation. Generated visuals run only for the creator-selected family. Automatic family selection, winner ranking, overlap resolution, NLE generation, final editing, and publishing remain excluded.
- **Accepted plugin pins:** MG `7ae59f1115da8a011113c81f31d320783b0ce8a4`; Illustrated Metaphor `48848affe018fc2cff8ee15bad7a09bb002776e4`; Hand-drawn Animation `624526f4dce0ba9794c1a717fa397eb3c7a1baad`. Identity, version, runner, timeout, clean-worktree requirement, and `enabled: false` semantics are unchanged.
- **Five-capability smoke:** Nexus marked Issue #15 PASS at runtime SHA `788ed3806c145189a51427f71a65013c050c5f48` for writing, insert materials, MG, Illustrated Metaphor, and Hand-drawn Animation. RC smoke reuse is allowed only by final exact diff/tree proof that capability runtime, Skills, and plugin configuration are unchanged; package `__version__` metadata does not alter those flows.
- **Release preparation:** package metadata and public `__version__` are `1.0.0`; canonical state, product requirements, roadmap, architecture/index, changelog, and ordinary-user release-note source are reconciled. Latest Formal Release remains `v0.6.1` until a separately authorized tag and GitHub Release exist.
- **Release-grade validation:** 879 Core tests passed with six expected opt-in skips; the complete audio-alignment eval and repeat digest passed; CLI sample/validate/prepare-draft passed; all six repository Skills passed `quick_validate.py`; exact pin/version/runner/timeout/default-disabled assertions passed; current roster count is exactly five; Issue #15 smoke runtime/config/Skill/renderer-source trees are identical to `788ed380…`; `git diff --check`, secret, private-artifact, generated-media, large-file, and Active Surface restricted-content gates passed.
- **Dependency audit disposition:** existing locked local renderer toolchains report npm advisories but no critical finding. The reported URL/config/archive and libheif conditions are not reached by the repository-controlled fixed-input flow; material acquisition accepts JPEG/PNG/WebP/SVG/PDF rather than HEIF. Dependency remediation is deliberately separated from this release-preparation diff so it cannot silently change renderer behavior or invalidate smoke evidence; Nexus may require a follow-up gate.
- **RC exact SHA:** the final PR head is the exact SHA under review and is recorded in the PR and Issue #16 handback. It is intentionally not duplicated as a literal inside the commit that defines itself, because changing this file would create a different SHA.
- **Known limitations:** no automatic selection/edit/publish capability; Product Architecture V2 Phase D/E/F have not started; MG Quality V2 remains future work; Phase 6 remains `TECHNICAL_DEMO_COMPLETED / HOLD_FOR_OWNER_REVIEW`; committed plugins remain disabled by default; locked local renderer dependency advisories remain documented for separate review.
- **Next gate:** complete release-grade verification on the final commit, push `release/v1.0.0-rc`, open an unmerged PR to `main`, then STOP for Nexus independent exact-SHA / PR Review. No tag, GitHub Release, package publication, or PR merge is authorized by this task.
