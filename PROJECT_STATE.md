...
AIGC:
  ...
  Label: '1'
  ProduceID: 'e58118be-46c1-439f-9ab5-79b0479d4d3e'
  PropagateID: 'e58118be-46c1-439f-9ab5-79b0479d4d3e'
  ReservedCode1: 'fcde10ab-9ed7-46f1-8349-7a305d90c0e4'
  ReservedCode2: 'fcde10ab-9ed7-46f1-8349-7a305d90c0e4'
---
...
| Latest Formal Release | [`v1.0.0`](docs/releases/v1.0.0.md), tagged `v1.0.0` on `main`; GitHub Release [v1.0.0](https://github.com/HWang0310/deep-talk-studio/releases/tag/v1.0.0). Previous formal release: [`v0.6.1`](docs/releases/v0.6.1.md), commit `8a0ac94cbaf6b2a472c3624c1c2e1f573cfb113d`. |
| Current Development Status | **v1.0.0 is formally released.** The release candidate passed Nexus exact-SHA / PR review (PR #17, RC `c7ff6972d0813d8d3a332ad1de1d1a6b35939240`) and was published as tag `v1.0.0`. Post-v1.0 work (Product Architecture V2 Phase D/E/F, MG Quality V2) remains unreleased and not started. |
| Product Code Baseline | Canonical development `agent/multi-asset-studio` at `788ed3806c145189a51427f71a65013c050c5f48`, merged into the v1.0.0 RC and released on `main`. |
...
### v1.0 Product Direction — One Main, Four Auxiliary

DeepTalk v1.0 is defined as **one main + four auxiliary** capabilities:

- **Main:** Research / Fact Check / Thesis / Reviewed Script writing.
- **Aux 1:** Source-backed insert materials (existing `prepare-materials` path).
- **Aux 2:** MG animation (`org.deeptalk.mg`, user-facing alias: `mg`).
- **Aux 3:** Illustrated Metaphor / 小黑漫画 (`org.deeptalk.illustrated-metaphor`, user-facing alias: `xiaohei`).
- **Aux 4:** Hand-drawn Animation (`org.deeptalk.handdrawn-animation`, user-facing alias: `handdrawn`).

The three generated visual plugins (Aux 2–4) are accessible through an explicit, invocation-scoped single-plugin entry point (CLI `visual-assist` + `src/deeptalk_studio/explicit_visual_assist.py`). A creator can say “这段做 MG / 小黑漫画 / 手绘动画” and only the explicitly selected plugin is invoked. No automatic family selection, winner ranking, or overlap resolution is performed. The V2 WHERE→WHAT→WHEN separation (Visual Opportunity, Candidate Portfolio, Placement Planner) is retained as future enhancement and is no longer a v1.0 release prerequisite.
...
### Product Architecture V2 — PASS / ACCEPTED; Phase A + Phase B + Phase C integrated
...
The V2 WHERE→WHAT→WHEN separation is **retained as future enhancement** and is **not a v1.0 release prerequisite**. The v1.0 release uses the explicit single-plugin entry point (`explicit_visual_assist.py`) built on top of the existing Contract V1 runtime.
...
This is a design document only. `Plan exists ≠ accepted; implemented ≠ released.` Product Architecture V2 has PASS / ACCEPTED; Phase A, Phase B, and Phase C are accepted and integrated. The V2 WHERE→WHAT→WHEN separation is retained as future enhancement, not a v1.0 release prerequisite. Formal Release is now `v1.0.0`; the v1.0 production path remains the released V1 path (the V2 multi-asset path is not production-enabled); Phase 6 remains `TECHNICAL_DEMO_COMPLETED / HOLD_FOR_OWNER_REVIEW`.
...
- **v1.0.0:** formally released **one main + four auxiliaries** — writing / Reviewed Script (main); source-backed insert materials, explicit MG, explicit Illustrated Metaphor / 小黑漫画, and explicit Hand-drawn Animation (auxiliaries). Creator authority, fail-closed source/rights/factual/timing/lineage/QA boundaries, and disabled-by-default plugin configuration are unchanged. See [docs/releases/v1.0.0.md](docs/releases/v1.0.0.md).
...
**v1.0.0 is published.** The release candidate passed Nexus exact-SHA / PR review (PR #17) and `main` now carries the released tree; tag `v1.0.0` and the GitHub Release exist at the canonical URL <https://github.com/HWang0310/deep-talk-studio/releases/tag/v1.0.0>. The explicit visual assist entry point (`explicit_visual_assist.py` + CLI `visual-assist`) is part of the released product and provides the creator-facing path for the three generated visual plugins (MG, Illustrated Metaphor, Hand-drawn Animation) without automatic orchestration. The current next step after release is Product Architecture V2 Phase D and later work, which remains unreleased and not started.
...
Core Phase 3B is ACCEPTED / IMPLEMENTED_UNRELEASED at accepted implementation SHA `ec595587a378d54bd2a18270ded504707b04ddea`. Core Phase 4 Candidate Asset Pack + Multi-option Edit Map is ACCEPTED / IMPLEMENTED_UNRELEASED at `817ca8b424f18714e4280d3990c1bc4221ec8dbe`. Phase 5 updates the independently accepted Hand-drawn revision to `624526f4dce0ba9794c1a717fa397eb3c7a1baad` while preserving MG `7ae59f1115da8a011113c81f31d320783b0ce8a4` and Illustrated Metaphor `48848affe018fc2cff8ee15bad7a09bb002776e4`. Its real-runner synthetic evidence proves order independence, failure isolation, Portfolio/Pack/map output, and basic creator usability across the three families. Phase 5 is **ACCEPTED / IMPLEMENTED_UNRELEASED**. All static config entries remain `enabled: false`; canonical development has been merged into `release/v1.0.0-rc`, reviewed, and released on `main` as `v1.0.0`; there is still no production enablement of the multi-asset V2 path and no new real Episode from this work.
